...
export default {
  ...
  name: "NavBar",
  computed: {
    ...mapGetters({getAuthToken:"userSessionManager/getAuthToken", getUserEmail:"userSessionManager/getUserEmail", getUserID:"userSessionManager/getUserID", isAdmin:"userSessionManager/isAdmin", isLoggedIn:"userSessionManager/isLoggedIn"}),
  },
  data() {
    return {
      signUpEmail: "",
  ...
  methods: {
    ...mapActions({registerUser: "userSessionManager/registerUser", loginUser: "userSessionManager/loginUser", logoutUser:"userSessionManager/logoutUser"}),
...
<nav class="bg-white border-gray-200 dark:bg-transparent">
    ...
    <div v-else class="flex items-center md:order-2">
        <div class="flex items-center">
            <router-link class="text-sm  text-blue-600 dark:text-blue-500 hover:underline" to="/sign_in">Login</router-link>
    ...
    <div class="items-center justify-between hidden w-full md:flex md:w-auto md:order-1" id="mobile-menu-2">
      <ul class="flex flex-col font-medium p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-transparent dark:border-gray-700">
        <li>
            <router-link class="block py-2 pl-3 pr-4 text-white bg-blue-700 rounded md:bg-transparent md:text-orange-700 md:p-0 md:dark:text-orange-500" to="/" aria-current="page">Home</router-link>
